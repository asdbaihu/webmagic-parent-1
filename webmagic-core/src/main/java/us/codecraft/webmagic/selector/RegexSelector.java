...
public class RegexSelector implements Selector {
    ...
    public RegexSelector(String regexStr, int group) {
        this.compileRegex(regexStr,0);
        this.group = group;
    }
    // by GLK
    public RegexSelector(String regexStr, int group,int flags) {
        this.compileRegex(regexStr,flags);
        this.group = group;
    }
    private void compileRegex(String regexStr,int flags) {
        if (StringUtils.isBlank(regexStr)) {
            throw new IllegalArgumentException("regex must not be empty");
        }
        try {
            //this.regex = Pattern.compile(regexStr, Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
            this.regex = Pattern.compile(regexStr,flags);// by GLK
